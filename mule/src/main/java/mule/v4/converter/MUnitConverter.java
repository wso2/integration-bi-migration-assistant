...
public class MUnitConverter {
    ...
    public static TextDocument convertTestSuite(Context ctx, String balFileName, TestSuite testSuite) {
        ...
        testSuite.beforeSuite().ifPresent(block ->
                intrinsics.add(buildAnnotatedFunction(ctx, imports, block, "@test:BeforeSuite")));
        testSuite.afterSuite().ifPresent(block ->
                intrinsics.add(buildAnnotatedFunction(ctx, imports, block, "@test:AfterSuite")));
        testSuite.beforeTest().ifPresent(block ->
                intrinsics.add(buildAnnotatedFunction(ctx, imports, block, "@test:BeforeEach")));
        testSuite.afterTest().ifPresent(block ->
                intrinsics.add(buildAnnotatedFunction(ctx, imports, block, "@test:AfterEach")));
    ...
    private static String buildTestFunction(Context ctx, Set<Import> imports, MUnitTest test) {
        ...
        if (!test.behavior().isEmpty()) {
            ...
            bodyLines.addAll(convertMUnitRecordsToLines(ctx, imports, test.behavior()));
        }
        if (!test.execution().isEmpty()) {
            bodyLines.add("    // Execution (Act)");
            bodyLines.addAll(convertMUnitRecordsToLines(ctx, imports, test.execution()));
        }
        if (!test.validation().isEmpty()) {
            bodyLines.add("    // Validation (Assert)");
            bodyLines.addAll(convertMUnitRecordsToLines(ctx, imports, test.validation()));
        }
    ...
    private static String buildAnnotatedFunction(Context ctx, Set<Import> imports,
                                                 LifecycleBlock block, String annotation) {
        List<String> bodyLines = convertMUnitRecordsToLines(ctx, imports, block.processors());
    ...
    private static List<String> convertMUnitRecordsToLines(Context ctx, Set<Import> imports,
                                                           List<MUnitRecord> records) {
        List<String> lines = new ArrayList<>();
        for (MUnitRecord record : records) {
            lines.addAll(convertMUnitRecordToLines(ctx, imports, record));
        }
        return lines;
    }

    private static List<String> convertMUnitRecordToLines(Context ctx, Set<Import> imports, MUnitRecord record) {
    ...
    private static List<Statement> convertMockWhen(MockWhen mockWhen) {
        ...
        xmlReconstructed.append("<munit-tools:mock-when processor=\"%s\"".formatted(mockWhen.processor()));
        ...
        if (!mockWhen.withAttributes().isEmpty()) {
            xmlReconstructed.append("    <munit-tools:with-attributes>\n");
            for (var attr : mockWhen.withAttributes()) {
                xmlReconstructed.append(
                        "        <munit-tools:with-attribute attributeName=\"%s\" whereValue=\"%s\"/>\n"
        ...
        mockWhen.thenReturn().ifPresent(thenReturn -> {
            xmlReconstructed.append("    <munit-tools:then-return>\n");
            thenReturn.payload().ifPresent(payload -> {
                xmlReconstructed.append("        <munit-tools:payload value='%s'".formatted(payload.value()));
                payload.mediaType().ifPresent(mt -> xmlReconstructed.append(" mediaType=\"%s\"".formatted(mt)));
                xmlReconstructed.append("/>\n");
            });
            for (var variable : thenReturn.variables()) {
                xmlReconstructed.append("        <munit-tools:variable key=\"%s\" value='%s'"
                        .formatted(variable.key(), variable.value()));
                variable.mediaType().ifPresent(mt -> xmlReconstructed.append(" mediaType=\"%s\"".formatted(mt)));
                xmlReconstructed.append("/>\n");
            }
            xmlReconstructed.append("    </munit-tools:then-return>\n");
        });

        xmlReconstructed.append("</munit-tools:mock-when>");
        return List.of(stmtFrom(
                ConversionUtils.wrapElementInTodoComment(xmlReconstructed.toString(), TODO_MOCK_DESC)));
    }
    ...
    private static List<Statement> convertSetEvent(Context ctx, SetEvent setEvent) {
        ...
        for (SetEventVariable variable : setEvent.variables()) {
            ...
            statements.add(stmtFrom("ctx.vars.%s = %s;".formatted(key, value)));
        }
    ...
    private static List<Statement> convertVerifyCall(VerifyCall verifyCall) {
        StringBuilder xmlReconstructed = new StringBuilder();
        xmlReconstructed.append("<munit-tools:verify-call processor=\"%s\"".formatted(verifyCall.processor()));
        ...
        xmlReconstructed.append("/>");
        return List.of(stmtFrom(
                ConversionUtils.wrapElementInTodoComment(xmlReconstructed.toString(), TODO_VERIFY_DESC)));
    }
    ...
    private static List<Statement> convertUnsupportedMUnitBlock(UnsupportedMUnitBlock unsupported) {
        return List.of(stmtFrom(
                ConversionUtils.wrapElementInTodoComment(unsupported.xmlBlock(), TODO_UNSUPPORTED_DESC)));
    }
    ...
    private static String convertInnerExpression(Context ctx, String inner) {
        ...
        if (inner.startsWith("vars.") || inner.startsWith("vars[")) {
            return Constants.VARS_FIELD_ACCESS + inner.substring("vars".length());
        }
