...
# Generate DataWeave samples (if dataweave directories exist)
if os.path.exists(dataweave_files_dir) and os.path.exists(dataweave_bal_files_dir):
    ...
    print(f'>> DataWeave directories not found for Mule 4.x, skipping DataWeave samples generation')
...
mule_v4_components_content = '(This section is AUTO-GENERATED by the test suite)\n\n'
mule_v4_components_content += 'The migration tool currently supports the following Mule 4.x components:\n\n'
for component in supported_components:
    component_title = component.replace("-", " ").title()
    component_link = component.lower().replace(" ", "-")
    mule_v4_components_content += f'- [{component_title}](docs/palette-item-mappings-v4.md#{component_link})\n'
mule_v4_components_content += '\n'
...
if has_mule_v4_section:
    ...
    while end_index < len(readme_content) and not readme_content[end_index].startswith('## '):
        end_index += 1
    readme_content = readme_content[:start_index] + [mule_v4_components_content] + readme_content[end_index:]
else:
    ...
    try:
        ...
        new_section = [mule_v4_components_section_header, mule_v4_components_content]
        ...
        readme_content.extend([mule_v4_components_section_header, mule_v4_components_content])
