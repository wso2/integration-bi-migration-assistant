...
relative_dw_md_path = 'Dataweave_samples.md'
...
print(f'DataWeave samples have been extracted and written to {dataweave_md_path}')
...
dataweave_section += 'The following DataWeave transformations and their corresponding Ballerina implementations are documented:\n\n'
for base_name in paired_files.keys():
    heading = generate_heading(base_name)
    dataweave_section += f'- [{heading}](Dataweave_samples.md#{heading.lower().replace(" ", "-")})\n'

dataweave_section += '\n'

# Update README.md
try:
    dw_start_index = readme_content.index('## DataWeave Transformations\n') + 1
    dw_end_index = dw_start_index
    while dw_end_index < len(readme_content) and not readme_content[dw_end_index].startswith('## '):
        dw_end_index += 1
    readme_content = readme_content[:start_index] + [new_components_list] + readme_content[end_index:dw_start_index] + [dataweave_section] + readme_content[dw_end_index:]
except ValueError:
    readme_content.append(dataweave_section)
...
print(f'DataWeave Transformations section has been updated in {readme_md_path}')