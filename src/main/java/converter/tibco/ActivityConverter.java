...
class ActivityConverter {
    ...
    private static List<BallerinaModel.Statement> convertReply(ActivityContext cx,
        ...
        if (reply.inputBindings().isEmpty()) {
            ...
            List<VarDeclStatment> inputBindings =
                    convertInputBindings(cx, input, reply.inputBindings());
    ...
    private static List<BallerinaModel.Statement> convertActivityExtension(ActivityContext cx,
        ...
        List<BallerinaModel.Statement> rest = switch (config) {
            case TibcoModel.Scope.Flow.Activity.ActivityExtension.Config.End ignored -> List.of(new Return<>(result));
    ...
    private static List<BallerinaModel.Statement> convertInvoke(ActivityContext cx,
        ...
        VarDeclStatment bindingCallResult =
        ...
        VarDeclStatment resultDecl =
                new VarDeclStatment(XML, cx.getAnnonVarName(), jsonToXMLFunctionCall);
        body.add(resultDecl);
        BallerinaModel.Expression.VariableReference result =
                new BallerinaModel.Expression.VariableReference(resultDecl.varName());
    ...
    private static VarDeclStatment createHTTPClientWithBasePath(ActivityContext cx,
        ...
        return new VarDeclStatment(cx.processContext.getTypeByName("http:Client"), cx.getAnnonVarName(), checkPanic);
    }
    ...
    private static VarAssignStatement addToContext(ActivityContext cx,
                                                   ...
                                                   String key) {
        assert !key.isEmpty();
        return new VarAssignStatement(new BallerinaModel.Expression.MemberAccess(cx.contextVarRef(), key), value);
    }
}
