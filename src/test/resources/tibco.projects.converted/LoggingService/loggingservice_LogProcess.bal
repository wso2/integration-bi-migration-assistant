...
function process_loggingservice_LogProcess(xml input) returns xml {
    ...
    worker start_worker {
        xml result0 = receiveEvent_12(input, context);
        result0 -> StartToLog;
        result0 -> StartToWriteFile;
        result0 -> StartToRenderXml;
        xml result1 = unhandled(input, context);
        xml result2 = unhandled_15(input, context);
        xml result3 = unhandled_16(input, context);
        xml result4 = unhandled_17(input, context);
    }
    worker LogToEnd {
    }
    ...
    xml result2 = <- unhandled_15_worker;
    xml result3 = <- unhandled_16_worker;
    xml result4 = <- unhandled_17_worker;
    xml result = result0 + result1 + result2 + result3 + result4;
    return result;
}

function receiveEvent_12(xml input, map<xml> context) returns xml {
    return input;
}

function unhandled(xml input, map<xml> context) returns xml { // comment
    //Unknown extension kind: bw.generalactivities.log
    return input;
}

function unhandled_15(xml input, map<xml> context) returns xml { // comment
    //Unknown extension kind: bw.file.write
    return input;
}

function unhandled_16(xml input, map<xml> context) returns xml { // comment
    //Unknown extension kind: bw.xml.renderxml
    return input;
}

function unhandled_17(xml input, map<xml> context) returns xml { // comment
    //Unknown extension kind: bw.file.write
    return input;
}
